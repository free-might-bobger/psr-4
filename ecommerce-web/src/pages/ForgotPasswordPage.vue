    ...
    <div class="reset-split">
      ...
      <div class="brand-panel">
        ...
        <div class="brand-content">
          ...
          <h2 class="brand-headline">Forgot your<br><span class="brand-accent">password?</span></h2>
          <p class="brand-desc">No worries — it happens to everyone. Enter your email and we'll send you a secure reset
            link right away.</p>
          <div class="brand-steps">
            ...
            <div class="brand-step">
              ...
              <span>Set your new password and sign in</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="form-panel">
        <div class="form-inner">
          <div class="form-breadcrumbs">
            <BreadCrumbsWrapper
              :bread-crumbs="[{ name: 'Login', path: '/login' }, { name: 'Forgot Password', path: '' }]" />
          </div>

          <!-- Sent success state -->
          <div v-if="emailSent" class="success-state">
            <div class="success-icon-wrap">
              <q-icon name="mark_email_read" size="36px" color="white" />
            </div>
            <h1 class="form-title">Check your inbox!</h1>
            <p class="form-subtitle">We've sent a password reset link to <strong>{{ sentEmail }}</strong>. Please check
              your inbox and spam folder.</p>
          ...
          <div v-else>
            <div class="form-header">
              <div class="form-header-icon">
                <q-icon name="lock_reset" size="24px" color="white" />
              </div>
              <div>
                <h1 class="form-title">Reset password</h1>
                <p class="form-subtitle">Enter your email to receive a secure reset link</p>
              </div>
            </div>

            <q-form class="reset-form" @submit.prevent="submitForgotPassword">
              <div class="field-group">
                <label class="field-label">Email address</label>
                <q-input v-model="forgotEmail" outlined dense type="email" placeholder="you@example.com"
...
const submitForgotPassword = async () => {
  ...
  try {
    await axios.post('forgot-password', { email: forgotEmail.value.trim() });
    sentEmail.value = forgotEmail.value.trim();
    emailSent.value = true;
  } catch (err: unknown) {
    const ax = err as { response?: { data?: { message?: string } } };
    $q.notify({
      message: ax.response?.data?.message ?? 'Something went wrong. Please try again.',
      type: 'negative',
      position: 'top',
      icon: 'error',
    });
